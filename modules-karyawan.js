...
function showKaryForm(id, p) {
  openModal(
    ...
    <div class="form-group" id="kStatusSubGroup" style="${p.status === 'nonaktif' ? '' : 'display:none'}"><label>Alasan Nonaktif</label><select class="form-control" id="kStatusSub">
        <option value="resign" ${p.statusSub === 'resign' ? 'selected' : ''}>Resign (Mengundurkan Diri)</option>
        <option value="PHK" ${p.statusSub === 'PHK' ? 'selected' : ''}>PHK (Pemutusan Hubungan Kerja)</option>
    </select></div>
    <div class="form-group"><label>Atasan</label><input class="form-control" id="kAtasan" value="${escHtml(p.atasan || '')}"></div></div></div>
...
function toggleNonaktifSub(val) {
  const group = document.getElementById('kStatusSubGroup');
  if (group) group.style.display = val === 'nonaktif' ? '' : 'none';
}
