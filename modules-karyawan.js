...
async function renderDashboard() {
  ...
  const [karyawan, cuti, absen, pengumuman, overtime, reimburse, dinas] =
    await Promise.all([
  ...
  if (isBOD) {
    karyawan.forEach((d) => {
      const k = d.data();
      gradeMapDash[(k.nama || "").toLowerCase()] = (
        k.gradeJabatan ||
        k.posisi ||
        ""
      ).toLowerCase();
  ...
  if (!totalPending)
    widgetLeft +=
      '<p class="text-sm" style="color:#999">Tidak ada pengajuan pending</p>';
  else {
  ...
  // Pengumuman
  widgetLeft +=
    '<div class="card"><div class="card-title mb-8">📢 Pengumuman Terbaru</div>';
  if (pengumuman.empty)
  ...
  if (hasAccess(4) && typeof loadPortalTeamReport === "function") {
    try {
      await loadPortalTeamReport(
        "dashTeamReportList",
        "_dashboardTeamDivFilter",
      );
    } catch (_e) {}
  }
...
async function renderDepartemen() {
  ...
  let h = "";
  if (snap.empty)
    h = `<tr><td colspan="${isBOD ? 4 : 5}" class="text-center">Belum ada</td></tr>`;
  else
    snap.forEach((d) => {
      const p = d.data();
    ...
    .then(function (d) {
      ...
      openModal(
        '<div class="modal-title">🏢 Detail Departemen</div>' +
          ...
          '<tr><td class="fw-700" style="padding:8px;width:120px">Nama</td><td style="padding:8px">' +
          escHtml(p.nama || "-") +
          "</td></tr>" +
          '<tr><td class="fw-700" style="padding:8px">Kode</td><td style="padding:8px">' +
          escHtml(p.kode || "-") +
          "</td></tr>" +
          '<tr><td class="fw-700" style="padding:8px">Kepala</td><td style="padding:8px">' +
          escHtml(p.kepala || "-") +
          "</td></tr>" +
          (p.createdAt
            ? '<tr><td class="fw-700" style="padding:8px">Dibuat</td><td style="padding:8px">' +
              formatDateTime(p.createdAt) +
              "</td></tr>"
            : "") +
          (p.updatedAt
            ? '<tr><td class="fw-700" style="padding:8px">Diupdate</td><td style="padding:8px">' +
              formatDateTime(p.updatedAt) +
              "</td></tr>"
            : "") +
          "</table>" +
          '<div style="margin-top:16px;display:flex;gap:8px">' +
          '<button class="btn btn-warning btn-sm" onclick="closeModalDirect();modalDepartemen(\'' +
          id +
          "')\">✏️ Edit</button>" +
...
async function syncDeptFromKaryawan() {
  ...
  toast(
    `Sinkronisasi selesai: ${added} departemen baru ditambahkan`,
    "success",
  );
  renderDepartemen();
}
...
async function renderPosisi() {
  ...
  let h = "";
  if (snap.empty)
    h = `<tr><td colspan="${isBOD ? 3 : 4}" class="text-center">Belum ada</td></tr>`;
  else
    snap.forEach((d) => {
      const p = d.data();
    ...
    .then(function (d) {
      ...
      openModal(
        '<div class="modal-title">💼 Detail Posisi</div>' +
          ...
          escHtml(p.nama || "-") +
          "</td></tr>" +
          '<tr><td class="fw-700" style="padding:8px">Departemen</td><td style="padding:8px">' +
          escHtml(p.departemen || "-") +
          "</td></tr>" +
          '<tr><td class="fw-700" style="padding:8px">Level</td><td style="padding:8px">' +
          escHtml(p.level || "-") +
          "</td></tr>" +
          (p.createdAt
            ? '<tr><td class="fw-700" style="padding:8px">Dibuat</td><td style="padding:8px">' +
          ...
          "</table>" +
          '<div style="margin-top:16px;display:flex;gap:8px">' +
          '<button class="btn btn-warning btn-sm" onclick="closeModalDirect();modalPosisi(\'' +
          id +
          "')\">✏️ Edit</button>" +
          '<button class="btn btn-outline btn-sm" onclick="closeModalDirect()">Tutup</button></div>',
        true,
...
async function syncPosFromKaryawan() {
  ...
  for (const [nama, dept] of positions) {
    if (!existing.has(nama)) {
      await db.collection("hrd_posisi").add({
        nama,
        departemen: dept,
        level: "",
        createdAt: new Date().toISOString(),
    ...
    .then(function (d) {
      ...
      openModal(
        '<div class="modal-title">🏛️ Detail Cabang</div>' +
          ...
          escHtml(p.nama || "-") +
          "</td></tr>" +
          '<tr><td class="fw-700" style="padding:8px">Kota</td><td style="padding:8px">' +
          escHtml(p.kota || "-") +
          "</td></tr>" +
          '<tr><td class="fw-700" style="padding:8px">Latitude</td><td style="padding:8px">' +
          (p.lat || 0) +
          "</td></tr>" +
          '<tr><td class="fw-700" style="padding:8px">Longitude</td><td style="padding:8px">' +
          (p.lng || 0) +
          "</td></tr>" +
          '<tr><td class="fw-700" style="padding:8px">Radius</td><td style="padding:8px">' +
          (p.radius || 10) +
          " meter</td></tr>" +
          (p.createdAt
            ? '<tr><td class="fw-700" style="padding:8px">Dibuat</td><td style="padding:8px">' +
          ...
          "</table>" +
          '<div style="margin-top:16px;display:flex;gap:8px">' +
          '<button class="btn btn-warning btn-sm" onclick="closeModalDirect();modalCabang(\'' +
          id +
          "')\">✏️ Edit</button>" +
          '<button class="btn btn-outline btn-sm" onclick="closeModalDirect()">Tutup</button></div>',
        true,
...
function filterKaryawan() {
  ...
  let h = "";
  if (!filtered.length)
    h = '<tr><td colspan="6" class="text-center">Tidak ada data</td></tr>';
  else
    filtered.forEach((k) => {
...
async function simpanKaryawan(id) {
  ...
  if (id) {
    ...
    if (
      data.kontrakMulai &&
      data.kontrakAkhir &&
      (data.kontrakMulai !== old.kontrakMulai ||
        data.kontrakAkhir !== old.kontrakAkhir)
    ) {
...
async function lihatHistoryKontrak(karyawanId) {
  ...
  kontrakSnap.forEach((d) => {
    ...
    rows.push({
      ...
      jenis:
        p.jenis === "kerja" ? "PKWT" : p.jenis === "tetap" ? "PKWTT" : p.jenis,
      mulai: p.mulai,
      akhir: p.berakhir,
      durasi: p.durasi,
    });
  ...
  if (!rows.length)
    h += '<p class="text-sm" style="color:#999">Belum ada riwayat kontrak</p>';
  else {
    h +=
...
async function detailKaryawan(id) {
  ...
  const kontrakSnap = await db
    .collection("hrd_kontrak")
    .where("karyawanId", "==", id)
    .get();
  // Also get contract documents from dokumen collection
  let kontrakDocs = [];
  kontrakSnap.forEach((kd) =>
    kontrakDocs.push({ id: kd.id, source: "kontrak", ...kd.data() }),
  );
  dokSnap.forEach((dd) => {
    const doc = dd.data();
  ...
  if (kontrakDocs.length) {
    ...
    kontrakHtml =
      '<p class="text-sm" style="color:#999">Belum ada kontrak.</p>';
  }
...
function switchImportTab(type, mode) {
  ...
  const el = document.getElementById(
    type === "kary" ? "importKaryTab" : "importGajiTab",
  );
  if (!el) return;
  if (mode === "api") {
  ...
  try {
    ...
    if (!resp.ok)
      throw new Error(
        "HTTP " +
          resp.status +
          " — Pastikan sheet bersifat publik (Anyone with the link can view)",
      );
    ...
    let h =
      '<div class="table-wrap" style="max-height:250px;overflow:auto"><table><thead><tr>';
    rows[0].forEach((col) => {
  ...
  if (!window._apiImportData) {
    ...
    try {
      ...
      if (text.trim().startsWith("<!"))
        return toast(
          'Response bukan CSV. Share sheet sebagai "Anyone with link"',
          "error",
        );
      window._apiImportData = text;
    } catch (e) {
...
async function processImportKaryawanFromText(text) {
  ...
  const map = {};
  headers.forEach((h, i) => {
    if (["nama", "name", "nama lengkap", "nama karyawan"].includes(h))
      map.nama = i;
    else if (
      ["nip", "employee id", "employee no", "id karyawan", "nik"].includes(h)
    )
      map.nip = i;
      ...
      map.telepon = i;
    else if (["departemen", "department", "divisi", "dept"].includes(h))
      map.departemen = i;
    else if (["posisi", "position", "jabatan", "job title"].includes(h))
      map.posisi = i;
    else if (
      [
        "tanggal masuk",
      ...
      ["gaji pokok", "salary", "basic salary", "gaji", "upah"].includes(h)
    )
      map.gajiPokok = i;
    else if (["atasan", "manager", "supervisor", "kepala"].includes(h))
      map.atasan = i;
    else if (["alamat", "address"].includes(h)) map.alamat = i;
    else if (["jenis kelamin", "gender", "kelamin"].includes(h))
      map.jenisKelamin = i;
    else if (["tempat lahir", "kota lahir"].includes(h)) map.tempatLahir = i;
    else if (["tanggal lahir", "tgl lahir", "birth date"].includes(h))
      map.tanggalLahir = i;
  });
  if (map.nama === undefined)
    return toast('Header harus berisi kolom "Nama"', "warning");
  let added = 0,
    updated = 0;
  for (let i = 1; i < rows.length; i++) {
    ...
    if (!nama) continue;
    const nip =
      map.nip !== undefined ? (row[map.nip] || "").trim() : generateNIP();
    const payload = {
      nama,
      nip,
      email: (map.email !== undefined ? row[map.email] || "" : "").trim(),
      telepon: (map.telepon !== undefined ? row[map.telepon] || "" : "").trim(),
      departemen: (map.departemen !== undefined
        ? row[map.departemen] || ""
        : ""
      ).trim(),
      posisi: (map.posisi !== undefined ? row[map.posisi] || "" : "").trim(),
      tanggalMasuk: (map.tanggalMasuk !== undefined
        ? row[map.tanggalMasuk] || ""
        : ""
      ).trim(),
      status:
        (map.status !== undefined ? row[map.status] || "" : "").trim() ||
        "aktif",
      gajiPokok:
        Number(
          String(map.gajiPokok !== undefined ? row[map.gajiPokok] : "").replace(
            /[^0-9.-]/g,
            "",
          ),
        ) || 0,
      ...
      jenisKelamin: (map.jenisKelamin !== undefined
        ? row[map.jenisKelamin] || ""
        : ""
      ).trim(),
      tempatLahir: (map.tempatLahir !== undefined
        ? row[map.tempatLahir] || ""
        : ""
      ).trim(),
      tanggalLahir: (map.tanggalLahir !== undefined
        ? row[map.tanggalLahir] || ""
        : ""
      ).trim(),
      updatedAt: new Date().toISOString(),
    };
...
async function renderStrukturOrg() {
  ...
  const active = karyawan.filter(
    (k) =>
      k.status !== "nonaktif" &&
      !(k.tipeKaryawan || "").toLowerCase().includes("resign"),
  );
  ...
  const founders = active.filter((k) =>
...
async function renderOnboarding() {
  ...
  if (!items.length) {
    ...
    items.forEach((p) => {
      ...
      const statusColor =
        pct === 100 ? "#2e7d32" : pct >= 50 ? "#f57f17" : "#c62828";
...
async function updateOnboarding(id) {
  ...
  await db
    ...
    .update({
      nama,
      tanggalMulai: document.getElementById("obEditTgl").value,
      checklist,
    });
  closeModalDirect();
    ...
    .then(function (d) {
      ...
      openModal(
        '<div class="modal-title">📝 Detail Lowongan</div>' +
          '<table style="width:100%;border-collapse:collapse">' +
          '<tr><td class="fw-700" style="padding:8px;width:120px;vertical-align:top">Posisi</td><td style="padding:8px">' +
          escHtml(p.posisi || "-") +
          "</td></tr>" +
          '<tr><td class="fw-700" style="padding:8px;vertical-align:top">Departemen</td><td style="padding:8px">' +
          escHtml(p.departemen || "-") +
          "</td></tr>" +
          '<tr><td class="fw-700" style="padding:8px;vertical-align:top">Status</td><td style="padding:8px"><span class="badge badge-' +
          (p.status === "open" ? "success" : "danger") +
          '">' +
          escHtml(p.status || "-") +
          "</span></td></tr>" +
          '<tr><td class="fw-700" style="padding:8px;vertical-align:top">Deadline</td><td style="padding:8px">' +
          formatDate(p.deadline) +
          "</td></tr>" +
          (p.deskripsi
            ? '<tr><td class="fw-700" style="padding:8px;vertical-align:top">Deskripsi</td><td style="padding:8px;white-space:pre-wrap">' +
              escHtml(p.deskripsi) +
              "</td></tr>"
            : "") +
          (p.createdAt
          ...
          "</table>" +
          '<div style="margin-top:16px;display:flex;gap:8px">' +
          '<button class="btn btn-warning btn-sm" onclick="closeModalDirect();modalLowongan(\'' +
          id +
          "')\">✏️ Edit</button>" +
    ...
    .then(function (d) {
      ...
      openModal(
        '<div class="modal-title">🧑‍💼 Detail Kandidat</div>' +
          '<table style="width:100%;border-collapse:collapse">' +
          '<tr><td class="fw-700" style="padding:8px;width:120px;vertical-align:top">Nama</td><td style="padding:8px">' +
          escHtml(p.nama || "-") +
          "</td></tr>" +
          '<tr><td class="fw-700" style="padding:8px;vertical-align:top">Posisi</td><td style="padding:8px">' +
          escHtml(p.posisi || "-") +
          "</td></tr>" +
          '<tr><td class="fw-700" style="padding:8px;vertical-align:top">Stage</td><td style="padding:8px"><span class="badge" style="background:' +
          stageColor +
          ';color:#fff">' +
          escHtml(p.stage || "Applied") +
          "</span></td></tr>" +
          '<tr><td class="fw-700" style="padding:8px;vertical-align:top">Email</td><td style="padding:8px">' +
          escHtml(p.email || "-") +
          "</td></tr>" +
          '<tr><td class="fw-700" style="padding:8px;vertical-align:top">Kontak</td><td style="padding:8px">' +
          escHtml(p.kontak || "-") +
          "</td></tr>" +
          (p.usia
            ? '<tr><td class="fw-700" style="padding:8px;vertical-align:top">Usia</td><td style="padding:8px">' +
              escHtml(p.usia) +
              " tahun</td></tr>"
            : "") +
          (p.jenisKelamin
            ? '<tr><td class="fw-700" style="padding:8px;vertical-align:top">Jenis Kelamin</td><td style="padding:8px">' +
              escHtml(p.jenisKelamin) +
              "</td></tr>"
            : "") +
          '<tr><td class="fw-700" style="padding:8px;vertical-align:top">DISC Pattern</td><td style="padding:8px">' +
          (p.discPattern
            ? '<span class="badge badge-primary">' +
              escHtml(p.discPattern) +
              "</span>"
            : "-") +
          "</td></tr>" +
          (p.discProfile
            ? '<tr><td class="fw-700" style="padding:8px;vertical-align:top">DISC Profile</td><td style="padding:8px">' +
              escHtml(p.discProfile) +
              "</td></tr>"
            : "") +
          (p.discScore
            ? '<tr><td class="fw-700" style="padding:8px;vertical-align:top">DISC Score</td><td style="padding:8px"><b>' +
              p.discScore +
              "</b></td></tr>"
            : "") +
          (p.sumber
            ? '<tr><td class="fw-700" style="padding:8px;vertical-align:top">Sumber</td><td style="padding:8px">' +
              escHtml(p.sumber) +
              "</td></tr>"
            : "") +
          (p.createdAt
            ? '<tr><td class="fw-700" style="padding:8px;vertical-align:top">Tanggal Masuk</td><td style="padding:8px">' +
              formatDateTime(p.createdAt) +
              "</td></tr>"
            : "") +
          "</table>" +
          '<div style="margin-top:16px;display:flex;gap:8px">' +
          '<button class="btn btn-warning btn-sm" onclick="closeModalDirect();modalKandidat(\'' +
          id +
          "')\">✏️ Edit</button>" +
          '<button class="btn btn-outline btn-sm" onclick="closeModalDirect()">Tutup</button>' +
          "</div>",
...
async function syncDiscCalonToKandidat() {
  const discSnap = await db
    ...
    .get();
  if (discSnap.empty)
    return toast("Tidak ada data DISC calon karyawan", "info");
...
async function renderJobdeskMgmt() {
  ...
  // Sort by nama
  window._jobdeskKaryawan.sort((a, b) =>
...
async function modalJobdeskPilih() {
  ...
  karySnap.forEach((d) => {
    const p = d.data();
    if ((p.status || "").toLowerCase() !== "nonaktif")
      list.push({ id: d.id, ...p });
  });
...
async function prosesUploadJobdesk(input) {
  ...
  try {
    ...
    if (ext === "txt") {
      ...
      toast(
        "Format file tidak didukung. Gunakan .txt, .pdf, atau .docx",
        "warning",
      );
    ...
    // Fill form fields
    if (parsed.deskripsi)
      document.getElementById("jdDesc").value = parsed.deskripsi;
    if (parsed.tanggungJawab)
      document.getElementById("jdTanggung").value = parsed.tanggungJawab;
    if (parsed.kualifikasi)
      document.getElementById("jdKualifikasi").value = parsed.kualifikasi;
    ...
    toast(
      "Dokumen berhasil dibaca! Periksa dan edit form sebelum menyimpan.",
      "success",
    );
  } catch (err) {
...
async function bacaPDFText(file) {
  ...
  if (!text || text.trim().length < 10) {
    throw new Error(
      "Tidak dapat mengekstrak teks dari PDF. Coba gunakan format .txt atau .docx",
    );
  }
  return text;
}
...
async function bacaDocxText(file) {
  ...
  throw new Error(
    "Tidak dapat mengekstrak teks dari DOCX. Coba gunakan format .txt",
  );
}
...
async function sinkronJobdeskKPI(karyawanId, nama) {
  ...
  try {
    ...
    if (jobdeskSnap.empty) {
      toast(
        "Simpan jobdesk terlebih dahulu sebelum sinkronisasi KPI",
        "warning",
      );
      return;
    }
    ...
    if (!targetKPI.trim()) {
      toast(
        "Target KPI belum diisi di jobdesk. Isi target KPI dulu lalu simpan.",
        "warning",
      );
      return;
    }
...
async function modalSinkronKPIBulk() {
  ...
  try {
    ...
    karyList.forEach((k) => {
      ...
      if (kpi) {
        ...
        if (skor >= 80) {
          ...
          statusBadge =
            '<span class="badge badge-warning">⚠️ Perlu Peningkatan</span>';
          countNeedImprove++;
        } else {
          statusBadge =
            '<span class="badge badge-danger">❌ Di Bawah Target</span>';
          countBelowTarget++;
        }
      } else {
        countNoKPI++;
    ...
    const totalEvaluated =
      countMeetTarget + countNeedImprove + countBelowTarget;
