...
async function renderDashboard() {
  ...
  main.innerHTML = `<div class="page-title"><span>${renderBackButton()}🏠 Beranda</span></div><div class="stats-grid" id="dashStats">Loading...</div><div class="grid-2" id="dashWidgets"></div>`;

  const [karyawan, cuti, pengumuman, overtime, reimburse, dinas] =
    await Promise.all([
...
async function loadDashBodTasks() {
  ...
  try {
    ...
    let h = "";
    if (snap.empty)
      h = '<p class="text-sm" style="color:#999">Tidak ada tugas aktif</p>';
    else {
      snap.forEach((d) => {
        const p = d.data();
...
async function showKaryawanForm(id, p) {
  ...
  depts.forEach(
    (d) =>
      (dOpts += `<option value="${d.data().nama}" ${p.departemen === d.data().nama ? "selected" : ""}>${d.data().nama}</option>`),
  );
  let pOpts = '<option value="">-- Pilih Posisi --</option>';
  posisi.forEach(
    (d) =>
      (pOpts += `<option value="${d.data().nama}" ${p.posisi === d.data().nama ? "selected" : ""}>${d.data().nama}</option>`),
  );
  let cOpts = '<option value="">-- Pilih Cabang --</option>';
  cabang.forEach(
    (d) =>
      (cOpts += `<option value="${d.data().nama}" ${p.cabang === d.data().nama ? "selected" : ""}>${d.data().nama}</option>`),
  );
...
function previewKaryawanFoto(input) {
  ...
  reader.onload = (e) => {
    ...
    img.onload = () => {
      ...
      document.getElementById("kyFotoPreview").innerHTML =
        `<img src="${window._kyFoto}" style="width:100%;height:100%;object-fit:cover">`;
    };
...
async function simpanKaryawan(id) {
  ...
  if (!data.nama || !data.nip)
    return toast("Nama & NIP wajib diisi", "warning");
...
async function simpanDept(id) {
  const data = {
    kode: document.getElementById("dpKode").value,
    nama: document.getElementById("dpNama").value,
    manager: document.getElementById("dpMgr").value,
    updatedAt: new Date().toISOString(),
  };
...
async function renderPosisi() {
  ...
  document.getElementById("tblPosisi").innerHTML =
    h || '<tr><td colspan="4" class="text-center">Belum ada</td></tr>';
}
...
function modalPosisi(id) {
  if (id)
    db.collection("hrd_posisi")
      .doc(id)
      .get()
      .then((d) => showPosisiForm(id, d.data()));
...
async function simpanPosisi(id) {
  const data = {
    ...
    gapok: Number(document.getElementById("psGapok").value) || 0,
    updatedAt: new Date().toISOString(),
  };
  if (!data.nama) return toast("Nama wajib", "warning");
...
async function renderCabang() {
  ...
  document.getElementById("tblCabang").innerHTML =
    h || '<tr><td colspan="3" class="text-center">Belum ada</td></tr>';
}
function modalCabang(id) {
  if (id)
    db.collection("hrd_cabang")
      .doc(id)
      .get()
      .then((d) => showCabangForm(id, d.data()));
...
async function simpanCabang(id) {
  const data = {
    nama: document.getElementById("cbNama").value,
    alamat: document.getElementById("cbAlamat").value,
    updatedAt: new Date().toISOString(),
  };
  if (!data.nama) return toast("Nama wajib", "warning");
...
function filterKaryawan() {
  ...
  filtered.forEach((k) => {
    const status =
      k.status === "aktif"
        ...
        : `<span class="badge badge-danger">Nonaktif (${k.status || "Resign"})</span>`;
  ...
  document.getElementById("tblKaryawan").innerHTML =
    h || '<tr><td colspan="8" class="text-center">Tidak ada data</td></tr>';
}
...
async function renderStrukturOrg() {
  ...
  if (!activeEmployees.length) {
    document.getElementById("orgWrap").innerHTML =
      '<div class="empty-state"><div class="icon">🌳</div><p>Belum ada data karyawan</p></div>';
    return;
  }
  ...
  function buildAvatar(person, borderColor) {
    const encodedName = encodeURIComponent(person.nama || "-").replace(
      /'/g,
      "%27",
    );
    const encodedPosisi = encodeURIComponent(person.posisi || "-").replace(
      /'/g,
      "%27",
    );
    if (person.foto) {
  ...
  function buildNode(person, tone, extraHtml) {
    ...
    const detail = extraHtml
      ? `<div class="org-node-extra">${extraHtml}</div>`
      : "";
  ...
  function findBestParent(person, parents) {
    ...
    parents.forEach((candidate) => {
      ...
      const overlap = childTokens.filter((token) =>
        parentTokens.includes(token),
      ).length;
      const deptMatch =
        normalizeText(candidate.departemen) &&
        normalizeText(candidate.departemen) === normalizeText(person.departemen)
          ? 2
          : 0;
...
async function renderOnboarding() {
  ...
  snap.forEach((d) => {
    ...
    const done = Array.isArray(p.checklist)
      ? p.checklist.filter((x) => x.done).length
      : 0;
  ...
  document.getElementById("tblOnboarding").innerHTML =
    h || '<tr><td colspan="4" class="text-center">Belum ada data</td></tr>';
}

function modalOnboarding(id) {
  if (id)
    db.collection("hrd_onboarding")
      .doc(id)
      .get()
      .then((d) => showOnboardingForm(id, d.data() || {}));
...
async function simpanOnboarding(id) {
  ...
  if (id) {
    const old =
      (await db.collection("hrd_onboarding").doc(id).get()).data() || {};
    const oldMap = {};
    (old.checklist || []).forEach(
      (x) => (oldMap[(x.task || "").trim().toLowerCase()] = !!x.done),
    );
    data.checklist = data.checklist.map((x) => ({
      ...x,
      done: oldMap[(x.task || "").trim().toLowerCase()] || false,
    }));
...
async function renderOffboarding() {
  ...
  document.getElementById("tblOffboarding").innerHTML =
    h || '<tr><td colspan="5" class="text-center">Belum ada data</td></tr>';
}

function modalOffboarding(id) {
  if (id)
    db.collection("hrd_offboarding")
      .doc(id)
      .get()
      .then((d) => showOffboardingForm(id, d.data() || {}));
...
async function simpanOffboarding(id) {
  ...
  if (id) {
    const old =
      (await db.collection("hrd_offboarding").doc(id).get()).data() || {};
    const oldMap = {};
    (old.checklist || []).forEach(
      (x) => (oldMap[(x.task || "").trim().toLowerCase()] = !!x.done),
    );
...
async function renderJobdeskMgmt() {
  ...
  snap.forEach((d) => {
    const p = d.data();
    const linkedKaryawanId =
      p.karyawanId || linkedKaryawanByUserId[p.userId] || p.userId;
    const karyawan =
      karyawanById[linkedKaryawanId] ||
      karyawanByName[(p.nama || "").toLowerCase().trim()] ||
      {};
  ...
  document.getElementById("tblJobdesk").innerHTML =
    h || '<tr><td colspan="5" class="text-center">Belum ada data</td></tr>';
}

function modalJobdesk(id) {
  if (id)
    db.collection("hrd_jobdesk")
      .doc(id)
      .get()
      .then((d) => showJobdeskForm(id, d.data() || {}));
...
async function simpanJobdesk(id) {
  ...
  try {
    if (data.userId) {
      ...
      if (userDoc.exists)
        resolvedKaryawanId = userDoc.data().linkedKaryawan || "";
    }
    if (!resolvedKaryawanId && data.userId) {
      const karyawanById = await db
        .collection("hrd_karyawan")
        .doc(data.userId)
        .get();
...
async function renderLowongan() {
  ...
  document.getElementById("tblLowongan").innerHTML =
    h || '<tr><td colspan="4" class="text-center">Belum ada lowongan</td></tr>';
}
...
function modalLowongan(id) {
  if (id)
    db.collection("hrd_lowongan")
      .doc(id)
      .get()
      .then((d) => showLowonganForm(id, d.data() || {}));
...
async function renderKandidat() {
  ...
  document.getElementById("tblKandidat").innerHTML =
    h || '<tr><td colspan="5" class="text-center">Belum ada kandidat</td></tr>';
}
...
function modalKandidat(id) {
  if (id)
    db.collection("hrd_kandidat")
      .doc(id)
      .get()
      .then((d) => showKandidatForm(id, d.data() || {}));
