...
function hitungStatusKesehatan() {
  ...
  var kolesterol = parseFloat(document.getElementById('tkfKolesterol').value) || 0;
  ...
  var rokokPerHari = parseFloat(document.getElementById('tkfRokokJumlah').value) || 0;
...
function generateCatatanOtomatis() {
  ...
  var hasAnyData = systole > 0 || diastole > 0 || nadi > 0 || suhu > 0 || bmi > 0;
  ...
  if (systole > 140) temuan.push('Tekanan darah tinggi/hipertensi (systole: ' + systole + ' mmHg)');
    ...
    temuan.push('Tekanan darah rendah/hipotensi (systole: ' + systole + ' mmHg)');
...
function renderForm(docId, data) {
  ...
  h += '<ol style="line-height:1.8;margin:0;padding-left:18px;font-size:.82rem;color:#333">';
  h +=
  ...
  h += '<option value="">-- Pilih --</option>';
  h += '<option value="A"' + (du.golonganDarah === 'A' ? ' selected' : '') + '>A</option>';
  h += '<option value="B"' + (du.golonganDarah === 'B' ? ' selected' : '') + '>B</option>';
  h += '<option value="AB"' + (du.golonganDarah === 'AB' ? ' selected' : '') + '>AB</option>';
  h += '<option value="O"' + (du.golonganDarah === 'O' ? ' selected' : '') + '>O</option>';
  h += '</select></div>';
  h +=
  ...
  h +=
    '<div style="background:#fff;border-radius:10px;padding:24px;margin-bottom:16px;box-shadow:0 1px 4px rgba(0,0,0,.06)">';
  h += '<h4 style="margin:0 0 8px;color:var(--primary)">B. Riwayat Kesehatan</h4>';
  h +=
  ...
  h += '<div style="display:flex;flex-wrap:wrap;gap:4px">' + diseaseChecks + '</div></div>';
  h += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">';
  h +=
  ...
  h +=
    '<div style="background:#fff;border-radius:10px;padding:24px;margin-bottom:16px;box-shadow:0 1px 4px rgba(0,0,0,.06)">';
  h += '<h4 style="margin:0 0 8px;color:var(--primary)">C. Pemeriksaan Fisik</h4>';
  h +=
  ...
  h +=
    '<div style="background:#fff;border-radius:10px;padding:24px;margin-bottom:16px;box-shadow:0 1px 4px rgba(0,0,0,.06)">';
  h += '<h4 style="margin:0 0 8px;color:var(--primary)">D. Pemeriksaan Lab (Opsional)</h4>';
  h +=
  ...
  h +=
    ...
    '>Tidak</option>';
  h += '<option value="ya"' + (km.gangguanMental === 'ya' ? ' selected' : '') + '>Ya</option>';
  h += '</select></div>';
  h +=
  ...
  h += '<option value="">-- Pilih --</option>';
  h += '<option value="rendah"' + (km.stres === 'rendah' ? ' selected' : '') + '>Rendah</option>';
  h += '<option value="sedang"' + (km.stres === 'sedang' ? ' selected' : '') + '>Sedang</option>';
  h += '<option value="tinggi"' + (km.stres === 'tinggi' ? ' selected' : '') + '>Tinggi</option>';
  h += '</select></div>';
  h +=
  ...
  h += '<option value="">-- Pilih --</option>';
  h += '<option value="baik"' + (km.kualitasTidur === 'baik' ? ' selected' : '') + '>Baik</option>';
  h +=
  ...
  h += '<option value="">-- Pilih --</option>';
  h += '<option value="tidak"' + (kb.merokok === 'tidak' ? ' selected' : '') + '>Tidak</option>';
  h += '<option value="ya"' + (kb.merokok === 'ya' ? ' selected' : '') + '>Ya</option>';
  h += '</select></div>';
  h +=
  ...
  h += '<option value="">-- Pilih --</option>';
  h += '<option value="tidak"' + (kb.alkohol === 'tidak' ? ' selected' : '') + '>Tidak</option>';
  h += '<option value="ya"' + (kb.alkohol === 'ya' ? ' selected' : '') + '>Ya</option>';
  h += '</select></div>';
  h +=
  ...
  h += '<option value="">-- Pilih --</option>';
  h += '<option value="tidak"' + (kb.olahraga === 'tidak' ? ' selected' : '') + '>Tidak</option>';
  h += '<option value="ya"' + (kb.olahraga === 'ya' ? ' selected' : '') + '>Ya</option>';
  h += '</select></div>';
  h +=
...
async function submitTestKesehatan(docId) {
  ...
  if (!hasSystole && !hasDiastole && !hasNadi && !hasSuhu)
    missingFields.push('Pemeriksaan Fisik (minimal salah satu: Systole/Diastole/Nadi/Suhu)');
  ...
  if (!kesimpulan.status) {
    toast('Harap isi Bagian C (Pemeriksaan Fisik) terlebih dahulu sebelum mengirim.', 'warning');
    return;
  }
