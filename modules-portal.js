  ...
  if (taskListEl) {
    ...
    if (!displayTasks.length) {
      ...
      displayTasks.forEach((t) => {
        ...
        if (isOverdue)
          th += ' <span style="font-size:.6rem;color:#c62828;font-weight:600">Terlambat</span>';
        if (t.assignedByName)
...
async function loadPortalBroadcastInfo() {
  try {
    ...
    if (snap.empty) {
      ...
      el.innerHTML = h || '<p class="text-sm" style="color:#999">Belum ada broadcast</p>';
...
async function loadPortalMeetingInfo() {
  try {
    ...
    if (!el) return;
    if (snap.empty) {
      el.innerHTML = '<p class="text-sm" style="color:#999">Belum ada meeting</p>';
      return;
    }
    let h = '';
    const items = [];
...
async function loadPortalInviteInfo() {
  try {
    ...
    if (!el) return;
    if (snap.empty) {
      el.innerHTML = '<p class="text-sm" style="color:#999">Belum ada undangan</p>';
      return;
    }
    let h = '';
    snap.forEach((d) => {
...
async function renderPortalPengumuman() {
  ...
  if (!items.length)
    h = '<p class="text-sm" style="color:#999;padding:16px">Belum ada pengumuman</p>';
...
async function renderPortalBroadcast() {
  ...
  if (!items.length)
    h = '<p class="text-sm" style="color:#999;padding:16px">Belum ada broadcast untuk Anda</p>';
...
async function renderPortalMeeting() {
  ...
  if (!myOnlineMeetings.length)
    onlineH = '<p class="text-sm" style="color:#999">Tidak ada meeting online aktif saat ini</p>';
  else
  ...
  const upcoming = items.filter(
    (p) => p.status !== 'selesai' && p.status !== 'dibatalkan' && p.tanggal >= today
  );
  ...
  if (!upcoming.length)
    h += '<p class="text-sm" style="color:#999">Tidak ada meeting mendatang</p>';
  else
    upcoming.forEach((p) => {
...
async function renderPortalOvertime() {
  ...
  let h = '';
  if (!items.length)
    h = '<tr><td colspan="5" class="text-center">Belum ada pengajuan overtime</td></tr>';
  else
    items.forEach((p) => {
      const badge =
        p.status === 'approved'
...
async function renderPortalCuti() {
  ...
  snap.forEach((d) => {
    const p = d.data();
    if (p.status === 'approved' && p.jenis === 'Cuti Tahunan') used += p.durasi || 1;
  });
...
async function loadPortalGaji() {
  ...
  let h = '';
  if (!items.length) h = '<tr><td colspan="6" class="text-center">Belum ada slip gaji</td></tr>';
  else
    items.forEach((p) => {
      const totPot =
        (p.bpjsKesehatan || 0) +
...
async function loadJobdesk() {
  ...
  let snap = await db
    ...
    .get();
  if (snap.empty)
    snap = await db.collection('hrd_jobdesk').where('userId', '==', currentUser.id).get();
  if (snap.empty)
    snap = await db.collection('hrd_jobdesk').where('karyawanId', '==', currentUser.id).get();
  ...
  if (snap.empty) {
    const kSnap = await db
      ...
      .get();
    if (!kSnap.empty) {
      snap = await db.collection('hrd_jobdesk').where('karyawanId', '==', kSnap.docs[0].id).get();
    }
  }
...
async function updateLamaran(id, status) {
  await db
    ...
    .doc(id)
    .update({ status, updatedBy: currentUser.nama, updatedAt: new Date().toISOString() });
...
async function renderPortalKPI() {
  ...
  items.sort((a, b) => (b.createdAt || '').localeCompare(a.createdAt || ''));
  let h = '';
  if (!items.length) h = '<tr><td colspan="6" class="text-center">Belum ada penilaian</td></tr>';
  else
    items.forEach((p) => {
