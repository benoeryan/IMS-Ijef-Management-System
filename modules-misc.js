...
async function hitungKPIIntegrasi(nama, periode) {
  ...
  const [karySnap, jobdeskSnap, absenSnap, penSnap, discSnap] =
    await Promise.all([
...
async function renderApprovalCenter() {
  ...
  items.forEach((item) => {
    const flow = flows.find(
      (f) => f.pengaju?.toLowerCase() === item.nama?.toLowerCase(),
    );
...
async function generateAllApprovalFlows() {
  ...
  for (const k of staffAndLeaders) {
    ...
    if (k.atasan && k.atasan.toLowerCase() !== "founder") {
      const atasan = karyawan.find(
        (a) => a.nama?.toLowerCase() === k.atasan?.toLowerCase(),
      );
      if (atasan)
        steps.push({
          nama: atasan.nama,
  ...
  toast(
    `${count} approval flow di-generate untuk ${staffAndLeaders.length} karyawan`,
    "success",
  );
  renderApprovalMgmt();
}
