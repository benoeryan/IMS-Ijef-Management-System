...
async function loadDailyTasks(filter) {
  ...
  if (filter === "today")
    ...
    filtered = _dailyTaskData.filter(
      (t) => t.assignedBy === myId && t.userId !== myId,
    );
  else if (filter === "history-assigned") {
    filtered = _dailyTaskData.filter((t) =>
      hasAccess(4)
        ? t.assignedBy && t.assignedBy !== t.userId
        : t.assignedBy === myId && t.userId !== myId,
    );
    ...
    filtered = _dailyTaskData.filter(
      (t) =>
        (t.type === "report" || t.title?.includes("Daily Report")) &&
        (t.userId === myId ||
          (t.targetUserName || "").toLowerCase().trim() === myName),
    );
  } else if (filter === "team-report" || filter === "all-report") {
    filtered = _dailyTaskData.filter(
      (t) => t.type === "report" || t.title?.includes("Daily Report"),
    );
    if (filter === "team-report" && !hasAccess(4)) {
      if (hasAccess(3))
        ...
        filtered = filtered.filter(
          (t) =>
            subs.includes((t.targetUserName || "").toLowerCase().trim()) ||
            t.userId === myId,
        );
      }
    }
      ...
      if (myDept) {
        items = items.filter(function (r) {
          ...
          const isOwn =
            ...
            (r.targetUserName || "").toUpperCase().trim() ===
              (currentUser.nama || "").toUpperCase().trim();
          return (
            isOwn ||
            d === myDept ||
            d.includes(myDept) ||
...
async function loadKaizenRecords(roles) {
  ...
  try {
    ...
    if (!hasAccess(3) && !isGA) {
      items = items.filter(
        (it) =>
          it.assignedBy === currentUser.id || it.userId === currentUser.id,
      );
    }
