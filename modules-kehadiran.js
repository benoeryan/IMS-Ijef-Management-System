...
const HARI_LIBUR_NASIONAL_2025 = [
  ...
  {
    tanggal: "2025-01-29",
    nama: "Tahun Baru Imlek 2576 Kongzili",
    tipe: "nasional",
  },
  {
    tanggal: "2025-03-29",
    nama: "Hari Raya Nyepi Tahun Baru Saka 1947",
    tipe: "nasional",
  },
  {
    tanggal: "2025-03-30",
    nama: "Hari Raya Idul Fitri 1446 H (Hari 1)",
    tipe: "nasional",
  },
  {
    tanggal: "2025-03-31",
    nama: "Hari Raya Idul Fitri 1446 H (Hari 2)",
    tipe: "nasional",
  },
  {
    tanggal: "2025-03-28",
  ...
  {
    tanggal: "2025-06-06",
    nama: "Hari Raya Idul Adha 1446 H",
    tipe: "nasional",
  },
...
const HARI_LIBUR_NASIONAL_2026 = [
  ...
  {
    tanggal: "2026-02-17",
    nama: "Tahun Baru Imlek 2577 Kongzili",
    tipe: "nasional",
  },
  {
    tanggal: "2026-03-19",
    nama: "Hari Raya Nyepi Tahun Baru Saka 1948",
    tipe: "nasional",
  },
  {
    tanggal: "2026-03-20",
    nama: "Hari Raya Idul Fitri 1447 H (Hari 1)",
    tipe: "nasional",
  },
  {
    tanggal: "2026-03-21",
    nama: "Hari Raya Idul Fitri 1447 H (Hari 2)",
    tipe: "nasional",
  },
  {
    tanggal: "2026-03-18",
  ...
  {
    tanggal: "2026-05-27",
    nama: "Hari Raya Idul Adha 1447 H",
    tipe: "nasional",
  },
  {
    tanggal: "2026-05-28",
    nama: "Cuti Bersama Idul Adha",
    tipe: "cuti_bersama",
  },
  {
    tanggal: "2026-05-29",
...
async function loadDailyTasks(filter) {
    ...
    snap.forEach((d) => {
      ...
      if (hasAccess(6)) {
        ...
        if (t.userId === myId || t.assignedBy === myId) {
          ...
          if (ownerLevel <= myLevel || ownerLevel === 0)
            _dailyTaskData.push({ id: d.id, ...t });
        }
      } else {
  ...
  if (filter === "today")
    ...
    if (hasAccess(4) && window._teamReportDivFilter) {
      filtered = filtered.filter((t) =>
        (t.departemen || "")
          .toUpperCase()
          .includes(window._teamReportDivFilter),
      );
    }
    ...
    if (window._teamReportCatFilter) {
      filtered = filtered.filter((t) => {
        ...
        if (fv === "tanpa kategori")
          return !t.kategori || t.kategori.trim() === "";
  ...
  const overdueTasks = _dailyTaskData.filter(
    (t) => t.tanggal < today && !t.done,
  ).length;
  ...
  if (filter === "team-report" || filter === "all-report") {
    ...
    if (filter === "team-report") {
      ...
      if (hasAccess(4) && window._teamReportDivFilter === "ACADEMIC") {
        ["Siswa", "Sensei", "Curriculum", "TSK-Job", "Tanpa Kategori"].forEach(
          (c) => {
    ...
    if (!filtered.length) {
      ...
      deptKeys.forEach(function (dept) {
        ...
        tasks.forEach(function (t) {
          ...
          historyHtml +=
            '<td class="fw-700">' +
            escHtml(t.targetUserName || t.userId || "-") +
            "</td>";
          ...
          historyHtml +=
            '<td class="text-sm">' + escHtml(t.assignedByName || "-") + "</td>";
  ...
  filtered.forEach((t) => {
    ...
    const priorityLabel =
      t.priority === "high"
        ...
        : t.priority === "low"
          ? "Rendah"
          : "Sedang";
    const taskProgress = t.done
      ? 100
      : Math.max(0, Math.min(100, parseInt(t.progress, 10) || 0));
    ...
    const isAssignedByOther =
      ...
      t.userId === currentUser.id;
    const isAssigner =
      t.assignedBy === currentUser.id && t.userId !== currentUser.id;
    const isFullAdmin = hasAccess(6);
    const isBODTask = currentUser.role === "bod";
    const canEdit =
      !isBODTask &&
      (isFullAdmin || (isOwn && !isAssignedByOther) || isAssigner);
    const canDelete =
      !isBODTask &&
      (isFullAdmin || (isOwn && !isAssignedByOther) || isAssigner);
    if (canEdit) {
  ...
  if (taskOnlyFilters.includes(filter) && hasAccess(2) && filtered.length) {
    ...
    if (taskItems.length) {
      ...
      taskItems.forEach(function (t) {
        var grpKey = hasAccess(4)
          ? t.departemen || "Tanpa Departemen"
          : t.targetUserName || t.assignedToName || "Saya";
      ...
      if (
        grpKeys.length > 1 ||
        (grpKeys.length === 1 && grpKeys[0] !== "Saya")
      ) {
        html +=
...
async function checkTaskReminders() {
  ...
  try {
    ...
    snap.forEach((d) => {
      const t = d.data();
      if (t.userId === currentUser.id && !t.done)
        tasks.push({ id: d.id, ...t });
    });
    ...
    Object.keys(byDept)
      ...
      .forEach(function (dept) {
        ...
        Object.keys(katMap)
          ...
          .forEach(function (kat) {
            ...
            items.forEach(function (r) {
              ...
              var progressColor =
                prog >= 100 ? "#2e7d32" : prog >= 50 ? "#f57f17" : "#c62828";

              // WA text with detail
              waText += "    \u2022 " + nama + "\n";
              waText += "      \ud83d\udcc8 Progress: " + prog + "%\n";
              waText +=
                "      \ud83d\udccb Aktivitas: " + aktivitasFirst + "\n";
              if (hasil)
                waText +=
                  "      \u2714 Hasil: " +
                  hasil.split("\n")[0].substring(0, 100) +
        ...
        htmlContent +=
          ...
          (deptItems.length
            ? Math.round((deptKendala / deptItems.length) * 100)
            : 0) +
        ...
        htmlContent +=
          ...
          (deptItems.length
            ? Math.round((deptNeedAttention / deptItems.length) * 100)
            : 0) +
          "%</b></div>";
        htmlContent += "</div>";
      });

    var avgProgress = reports.length
      ? Math.round(totalProgressValue / reports.length)
      : 0;
    var highCoverage = reports.length
      ? Math.round(((totalDone + totalOnTrack) / reports.length) * 100)
      : 0;
    var kendalaCoverage = reports.length
      ? Math.round((totalKendala / reports.length) * 100)
      : 0;
    waText +=
      "\ud83d\udcca Total: " +
      reports.length +
    ...
    Object.keys(groups)
      ...
      .forEach(function (div) {
        ...
        rows.forEach(function (r) {
          ...
          html +=
            '<div style="flex:1"><div class="fw-700">' +
            escHtml(pic) +
            "</div>";
          html +=
        ...
        if (hasAccess(2)) {
          ...
          var wrKendalaCov = rows.length
            ? Math.round((wrKendala / rows.length) * 100)
            : 0;
          html +=
            ...
            "</b></span>" +
            "</div>" +
            '<div style="color:#2e7d32;font-weight:700">\ud83d\udcc8 Rata-rata Progress: ' +
            wrAvg +
            "%</div>" +
            '<div style="color:#777;margin-top:2px">Coverage kendala: <b>' +
            wrKendalaCov +
            "%</b> laporan punya hambatan</div>" +
            "</div>";
        }
        html += "</div>";
      });
