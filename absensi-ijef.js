...
if (
  !window._pageResourceCleanupHandlers.includes(handleAbsensiResourceCleanup)
) {
...
async function getCurrentAbsensiIdentity() {
  const ids = new Set(
    [currentUser.id, currentUser.linkedKaryawan].filter(Boolean),
  );
  const names = new Set(
    [(currentUser.nama || "").toLowerCase().trim()].filter(Boolean),
  );
  try {
...
async function getCurrentUserAbsensiRecords(forceRefresh) {
  ...
  const snaps = await Promise.all(
    identity.ids.map((id) =>
      db.collection("hrd_absensi").where("userId", "==", id).get(),
    ),
  );
  const seen = new Set();
  const records = [];
...
function renderAbsensiIJEF() {
  ...
  const isGM =
    (currentUser.posisi || "").toUpperCase().includes("GENERAL MANAGER") ||
    (currentUser.posisi || "").toUpperCase() === "GM";
  const isAdmin = currentUser.role === "admin";
  const isGMorAdmin = isAdmin || isGM;

  const syncBtn = isGMorAdmin
  ...
  navigator.geolocation.getCurrentPosition(
    async (pos) => {
      try {
        ...
        if (locStatus.allowed) {
          ...
          try {
            const { masuk: hasMasuk, pulang: hasPulang } =
              await getTodayAttendanceState();
            if (!hasMasuk) {
    ...
    (err) => {
      ...
      if (err.code === 1) {
        // PERMISSION_DENIED
        msg =
          "Izin Lokasi Ditolak. Harap aktifkan GPS & Izin Lokasi di browser/pengaturan HP Anda agar bisa absen.";
      }
      ...
      if (err.code === 1) {
        toast(
          "Harap aktifkan Izin Lokasi di pengaturan Browser/HP Anda",
          "error",
        );
      }
    },
  ...
  if (currentGPS) {
    try {
      ...
      if (locStatus.allowed || flexUser) {
        ...
        if (!hasMasuk) {
          btn.textContent = isFlexMode
            ? "⏰ ABSEN MASUK (Flexible + GPS)"
          ...
          btn.textContent = isFlexMode
            ? "🏠 ABSEN PULANG (Flexible + GPS)"
            : "🏠 ABSEN PULANG (Selfie + GPS)";
          btn.style.background = "var(--warning)";
          btn.disabled = false;
          if (autoEl)
            autoEl.innerHTML = isFlexMode
...
async function loadTodayHistory() {
  ...
  docs.forEach((p) => {
    ...
    const nameMatch = identity.names.includes(
  ...
  if (tab === "pengajuan") {
    ...
    snap.forEach((d) => {
      ...
      const steps = getApprovalStepsForItem(
        flows,
        p,
        getApprovalCategory("hrd_dinas_luar", p),
      );
      ...
      const canApprove =
        isPending && hasAccess(3) && currentUser.role !== "bod" && isMyTurn;
...
async function loadRekapGrid() {
  ...
  try {
    ...
    cutiSnap.forEach((d) => {
      ...
      const uids = [c.userId, (c.nama || "").toLowerCase().trim()].filter(
        Boolean,
      );
    ...
    overtimeSnap.forEach((d) => {
      ...
      if (
        ...
        return;
      const uids = [o.userId, (o.nama || "").toLowerCase().trim()].filter(
        Boolean,
      );
    ...
    const processDinas = (dl) => {
      ...
      if (!startD) return;
      const uids = [dl.userId, (dl.nama || "").toLowerCase().trim()].filter(
        Boolean,
      );
    ...
    const filteredUsers = !hasAccess(3)
      ? users.filter(
          (u) =>
            u.nama?.toLowerCase() === currentUser.nama?.toLowerCase() ||
            u.id === currentUser.id,
    ...
    absenSnap.forEach((d) => {
      ...
      const uids = [p.userId, (p.nama || "").toLowerCase().trim()].filter(
        Boolean,
      );
      uids.forEach((uid) => {
    ...
    dateList.forEach((dt) => {
      ...
      const hdrStyle =
        isWknd || liburSet.has(ds) ? "background:#9e9e9e;color:#fff;" : "";
    ...
    filteredUsers.forEach((u) => {
      ...
      const userDinas = {
        ...(dinasLuarMap[u.id] || {}),
        ...(dinasLuarMap[namaLow] || {}),
      };
      const userOT = { ...(otMap[u.id] || {}), ...(otMap[namaLow] || {}) };

      const userRawAbsen = absenSnap.docs
        .filter((d) => {
          const p = d.data();
          return (
            (p.userId === u.id ||
              (p.nama && p.nama.toLowerCase() === namaLow)) &&
            p.tanggal >= startDate &&
            p.tanggal <= endDate
          );
        })
      ...
      dateList.forEach((dt) => {
        ...
        const hasMasuk = userRawAbsen.some(
          (a) => a.tanggal === ds && a.tipe === "masuk",
        );
        const hasPulang = userRawAbsen.some(
          (a) => a.tanggal === ds && a.tipe === "pulang",
        );
        ...
        if (cutiStatus) {
          const cTypes = {
            WFH: ["#009688", "W", "WFH"],
            "Cuti Sakit": ["#e91e63", "S", "Cuti Sakit"],
            "Izin Pribadi": ["#ffc107", "I", "Izin Pribadi"],
            "Cuti Melahirkan": ["#9c27b0", "M", "Cuti Melahirkan"],
          };
          ...
          color = "#ffebee";
          text = '<span style="color:#f44336">⚠️</span>';
          title = ' title="Mangkir (Tanpa Keterangan)"';
        } else if (isLibur) {
          color = "#9e9e9e";
          text = "H";
        ...
        if (flex.enabled && jamKerja)
          title = (title || "") + ` title="${jamKerja.toFixed(1)} jam"`;
  ...
  try {
    ...
    if (flex.enabled) {
      if (flex.jamMasukMax) {
        const [hMax, mMax] = flex.jamMasukMax.split(":").map(Number);
        statusEl.value =
          waktuMasuk > hMax * 60 + mMax ? "terlambat" : "tepat_waktu";
      }
    } else {
...
async function hapusAbsenHari() {
  ...
  await batch.commit();
  toast(
    `${toDelete.length} record dihapus. Jangan lupa klik "Sinkronisasi" di menu Penggajian.`,
    "success",
  );
  closeModalDirect();
  setTimeout(() => loadRekapGrid(), 500);
...
async function doGenerateAbsensi() {
  ...
  const autoPayroll =
    document.getElementById("genAutoPayroll")?.checked || false;

  if (!startDate || !endDate)
    return toast("Isi tanggal mulai dan selesai", "warning");
  if (
    !confirm(
      `Generate absensi dari ${startDate} s/d ${endDate}?\nSistem akan mengisi kehadiran hari kerja dan mengkalkulasi slip gaji.`,
    )
  )
    return;
