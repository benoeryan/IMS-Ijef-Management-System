"use strict";
// CORE.JS — HRD & Legal IJEF Corp v16.1.5
...
async function autoLoadHariLiburNasional() {
  ...
  for (const year of years) {
    const dataToSync =
      year === 2025 ? HARI_LIBUR_NASIONAL_2025 : HARI_LIBUR_NASIONAL_2026;
    try {
    ...
    .onSnapshot((doc) => {
      if (doc.exists) {
        ...
        if (
          ...
          parseFloat(remoteVersion) > parseFloat(APP_VERSION)
        ) {
          console.log(
            `[UPDATE] New version detected: ${remoteVersion}. Current: ${APP_VERSION}. Reloading...`,
          );
...
function hasHeadLevelAccess() {
  ...
  return (
    posisi.includes("HEAD") ||
    posisi.includes("KEPALA") ||
    posisi.includes("GENERAL MANAGER") ||
    posisi === "GM"
  );
}
...
function loadScriptOnce(src) {
  ...
  window._routeScriptPromises[src] = new Promise((resolve, reject) => {
    ...
    ).find((el) => el.getAttribute("data-lazy-src") === src);
    if (existing) {
...
function navigateTo(page) {
  ...
  const routes = {
    ...
    "portal-perjalanan-dinas": "renderPerjalananDinas",
...
function doesTaskBelongToUser(task, user) {
  ...
  if (userId && (task.userId === userId || task.karyawanId === userId))
    return true;
...
function isDailyReportEntry(task) {
  return (
    task?.type === "report" ||
    String(task?.title || "").includes("Daily Report")
  );
}
...
function getApprovalCategory(col, data) {
  ...
  if (col === "hrd_reimbursement" || col === "hrd_reimburse_dinas")
...
function getApprovalStepsForItem(flows, itemOrName, category) {
  ...
  if (Array.isArray(item.approvalFlow) && item.approvalFlow.length) {
    if (Array.isArray(item.approvalFlow[0]?.steps)) {
      const embeddedFlow =
        (itemCategory &&
          item.approvalFlow.find((flow) => flow?.jenis === itemCategory)) ||
        item.approvalFlow[0];
...
async function showSystemNotification(title, body) {
  ...
  if ("serviceWorker" in navigator) {
    try {
      ...
      console.warn(
        "[Notif] SW showNotification failed, fallback to standard:",
        e.message,
      );
    }
  }
