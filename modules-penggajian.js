...
function filterGajiTable() {
  const q = (
    document.getElementById("filterNamaGaji")?.value || ""
  ).toLowerCase();
  ...
  let h = "",
    ...
    count = 0;
  if (!filtered.length)
    h = '<tr><td colspan="12" class="text-center">Tidak ada data</td></tr>';
  else
    filtered.forEach((p) => {
...
async function hapusSelectedGaji() {
  const ids = [];
  document
    .querySelectorAll(".gaji-cb:checked")
...
async function doGenerateAllGaji(
  ...
  const incTunj = forcedSelections
    ? forcedSelections.tunj
    : document.getElementById("genIncTunj")
      ? document.getElementById("genIncTunj").checked
      : true;
  const incInsentif = forcedSelections
    ? forcedSelections.insentif
    : document.getElementById("genIncInsentif")
      ? document.getElementById("genIncInsentif").checked
      : true;
  const incReimb = forcedSelections
    ? forcedSelections.reimb
    : document.getElementById("genIncReimb")
      ? document.getElementById("genIncReimb").checked
      : true;
  const incKasbon = forcedSelections
    ? forcedSelections.kasbon
    : document.getElementById("genIncKasbon")
      ? document.getElementById("genIncKasbon").checked
      : true;

  const incTunjCuti = document.getElementById("genIncTunjCuti")
    ? document.getElementById("genIncTunjCuti").checked
    : false;
  const incBPJSKes = forcedSelections
    ? forcedSelections.bpjsKes
    : document.getElementById("genIncBPJSKes")
      ? document.getElementById("genIncBPJSKes").checked
      : true;
  const incBPJSTK = forcedSelections
    ? forcedSelections.bpjsTK
    : document.getElementById("genIncBPJSTK")
      ? document.getElementById("genIncBPJSTK").checked
      : true;
  const incPPH = forcedSelections
    ? forcedSelections.pph
    : document.getElementById("genIncPPH")
      ? document.getElementById("genIncPPH").checked
      : true;

  if (!isAuto) closeModalDirect();
  if (
    !isAuto &&
    !confirm("Konfirmasi: Generate slip gaji untuk semua karyawan aktif?")
  )
    return;
  ...
  try {
    const bulan =
      forcedBulan ||
      document.getElementById("filterBulanGaji")?.value ||
      monthStr();
    ...
    const totalKalender =
      Math.round((dEnd - dStart) / (1000 * 60 * 60 * 24)) + 1;
    ...
    const [
      ...
      db
        ...
        .where("tanggal", ">=", periodeStart)
        .where("tanggal", "<=", periodeEnd)
        .get(),
      db
      ...
      db
        ...
        .where("status", "in", ["aktif", "approved"])
        .get(),
    ...
    liburSnap.forEach((d) => {
      const h = d.data();
      if (h.tanggal >= periodeStart && h.tanggal <= periodeEnd)
        holidays.add(h.tanggal);
    });
    ...
    offboardingSnap.forEach((d) => {
      const o = d.data();
      if (o.tanggalKeluar)
        resignMap[(o.nama || "").toLowerCase().trim()] = o.tanggalKeluar;
    });
    ...
    for (const k of kDocs) {
      ...
      if (!isFullMonth) {
        gajiPokok = Math.round(
          (hariAktifKalender / totalKalender) * (Number(k.gajiPokok) || 0),
        );
      }
      ...
      absenList.forEach((a) => {
        ...
        if (
          (a.userId === k.id ||
            aNama === namaLow ||
            aNama.includes(namaLow) ||
            namaLow.includes(aNama)) &&
          a.tanggal >= rangeStart &&
          a.tanggal <= rangeEnd
        ) {
      ...
      cutiList.forEach((c) => {
        ...
        const isMatch =
          c.userId === k.id ||
          cNama === namaLow ||
          namaLow.includes(cNama) ||
          cNama.includes(namaLow);
        if (isMatch) {
          const start = c.mulai;
      ...
      dinasList.forEach((dl) => {
        ...
        if (
          dl.userId === k.id ||
          dNama === namaLow ||
          dNama.includes(namaLow) ||
          namaLow.includes(dNama)
        ) {
      ...
      let hadirCount = 0,
        mangkirCount = 0;
      for (
        let dt = new Date(rangeStart + "T00:00:00");
        dt <= new Date(rangeEnd + "T00:00:00");
        dt.setDate(dt.getDate() + 1)
      ) {
        const ds = getSafeDateString(dt);
        if (isWorkDay(ds)) {
      ...
      const potonganMangkir = isExempt
        ? 0
        : Math.round(
            (mangkirCount / totalKalender) * (Number(k.gajiPokok) || 0),
          );
      ...
      otList.forEach((o) => {
        ...
        if (
          (o.userId === k.id || oNama === namaLow || oNama.includes(namaLow)) &&
          o.tanggal >= periodeStart &&
          o.tanggal <= periodeEnd
        ) {
      ...
      if (incTunj) {
        tunjList.forEach((t) => {
          ...
          if (
            p === "semua" ||
            p === "all" ||
            p.split(",").some((x) => namaLow.includes(x.trim()))
          ) {
      ...
      if (incInsentif) {
        insentifList.forEach((ins) => {
          ...
          const isNameMatch =
            insNama === namaLow ||
            namaLow.includes(insNama) ||
            insNama.includes(namaLow);
          if (isNameMatch) {
      ...
      if (incReimb) {
        reimbList.forEach((r) => {
          ...
          if (
            (rNama === namaLow || namaLow.includes(rNama)) &&
            rDate >= periodeStart &&
            rDate <= periodeEnd
          ) {
      ...
      const bpjsKes = incBPJSKes
        ? Math.round((Number(k.gajiPokok) || 0) * 0.01)
        : 0;
      const bpjsTK = incBPJSTK
        ? Math.round((Number(k.gajiPokok) || 0) * 0.02)
        : 0;
      const bruto =
        gajiPokok +
        tunjTetap +
      ...
      let pph21 = 0;
      if (incPPH) {
        const nettoTahunan = Math.max(
          0,
          ((Number(k.gajiPokok) || 0) + tunjTetap - bpjsKes - bpjsTK) * 12 -
            54000000,
        );
        let pphT = 0;
        if (nettoTahunan <= 60000000) pphT = nettoTahunan * 0.05;
        else if (nettoTahunan <= 250000000)
          pphT = 3000000 + (nettoTahunan - 60000000) * 0.15;
...
function hitungGaji() {
  ...
  const tTransport =
    Number(document.getElementById("gjTunjTransport").value) || 0;
  ...
  else if (tahunan <= 250000000)
    pphTahunan = 60000000 * 0.05 + (tahunan - 60000000) * 0.15;
  else if (tahunan <= 500000000)
    pphTahunan =
      60000000 * 0.05 + 190000000 * 0.15 + (tahunan - 250000000) * 0.25;
  else
    pphTahunan =
      60000000 * 0.05 +
      190000000 * 0.15 +
      250000000 * 0.25 +
      (tahunan - 500000000) * 0.3;
...
async function autoFillGajiFromKaryawan() {
  ...
  let tunjTotal = 0;
  for (const d of tunjSnap) {
    const t = d.data();
    const penerima = (t.penerima || "Semua").toLowerCase();
    if (penerima === "semua" || penerima.includes(nama.toLowerCase()))
      tunjTotal += t.nominal || 0;
  }
  ...
  if (kpiFound && kpiScore > 0) {
    ...
    document.getElementById("gjInsentif").value = Math.round(
      gaji * insentifPct,
    );
  }
  ...
  if (totalOTJam > 0) {
    ...
    lemburNominal = Math.round(
      jam1 * gajiPerJam * 1.5 + jamSisa * gajiPerJam * 2,
    );
...
async function processImportPenggajianFromText(text) {
  ...
  const map = {};
  headers.forEach((h, i) => {
    if (["nama", "name", "nama karyawan", "employee name"].includes(h))
      map.nama = i;
    else if (
      ["periode", "period", "bulan", "month", "periode gaji"].includes(h)
    )
      map.periode = i;
    else if (
      ["gaji pokok", "salary", "basic salary", "gaji", "upah pokok"].includes(h)
    )
      map.gajiPokok = i;
    else if (
      ["tunjangan", "allowance", "tunjangan total", "total tunjangan"].includes(
        h,
      )
    )
      map.tunjangan = i;
    else if (
      ["potongan", "deduction", "deductions", "total potongan"].includes(h)
    )
      map.potongan = i;
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (!nama) continue;
    const periode =
      map.periode !== undefined ? (row[map.periode] || "").trim() : monthStr();
    if (!periode) continue;
    const gajiPokok =
      Number(
    ...
    const tunjangan =
      Number(
        String(map.tunjangan !== undefined ? row[map.tunjangan] : "").replace(
          /[^0-9.-]/g,
          "",
        ),
      ) || 0;
    const lembur =
      Number(
        String(map.lembur !== undefined ? row[map.lembur] : "").replace(
          /[^0-9.-]/g,
          "",
        ),
      ) || 0;
    const bonus =
      Number(
        String(map.bonus !== undefined ? row[map.bonus] : "").replace(
          /[^0-9.-]/g,
          "",
        ),
      ) || 0;
    const potongan =
      Number(
        String(map.potongan !== undefined ? row[map.potongan] : "").replace(
          /[^0-9.-]/g,
          "",
        ),
      ) || 0;
    const pph21 =
      Number(
        String(map.pph21 !== undefined ? row[map.pph21] : "").replace(
          /[^0-9.-]/g,
          "",
        ),
      ) || 0;
    let totalBersih =
      Number(
        String(
          map.totalBersih !== undefined ? row[map.totalBersih] : "",
        ).replace(/[^0-9.-]/g, ""),
      ) || 0;
    if (!totalBersih)
      totalBersih = gajiPokok + tunjangan + lembur + bonus - potongan - pph21;
    const payload = {
      nama,
      periode,
      gajiPokok,
...
async function lihatSlip(id) {
  try {
    ...
    let potDetailHtml = "";
    if (p.potonganMangkir)
      potDetailHtml += `<tr><td>Pot. Absen / Mangkir (${p.mangkirHari || 0} hari)</td><td style="text-align:right;color:var(--danger)">-${formatCurrency(p.potonganMangkir)}</td></tr>`;
    if (p.bpjsKesehatan)
      potDetailHtml += `<tr><td>BPJS Kesehatan (1%)</td><td style="text-align:right;color:var(--danger)">-${formatCurrency(p.bpjsKesehatan)}</td></tr>`;
    if (p.bpjsTK)
      potDetailHtml += `<tr><td>BPJS TK (2%)</td><td style="text-align:right;color:var(--danger)">-${formatCurrency(p.bpjsTK)}</td></tr>`;
    if (p.kasbon)
      potDetailHtml += `<tr><td>Kasbon/Loan</td><td style="text-align:right;color:var(--danger)">-${formatCurrency(p.kasbon)}</td></tr>`;
    if (p.pph21)
      potDetailHtml += `<tr><td>PPH 21</td><td style="text-align:right;color:var(--danger)">-${formatCurrency(p.pph21)}</td></tr>`;

    // Build reimbursement evidence list
    let reimbDetailHtml = "";
    if (p.reimbursementDetails && p.reimbursementDetails.length > 0) {
      reimbDetailHtml =
        '<div class="mt-16" style="border-top:1px solid #ddd; padding-top:8px"><div class="fw-700 text-xs color-primary mb-4">📎 Bukti Reimbursement:</div><div style="display:flex; gap:8px; flex-wrap:wrap">';
      p.reimbursementDetails.forEach((r) => {
        const fileData = encodeURIComponent(
          JSON.stringify({ name: r.judul, type: "image/jpeg", data: r.bukti }),
        );
        reimbDetailHtml += `
...
async function renderKasbon() {
  ...
  let h = "";
  if (!items.length)
    h = '<tr><td colspan="10" class="text-center">Belum ada</td></tr>';
  else
    items.forEach((p) => {
...
function calcKasbonPreview() {
  ...
  document.getElementById("kbAngsuranPreview").textContent = formatCurrency(
    Math.ceil(jml / cic),
  );
}
...
async function viewKasbon(id) {
  ...
  if (p.paymentHistory && p.paymentHistory.length) {
    ...
    p.paymentHistory.forEach((ph, i) => {
      const tgl = ph.tanggal
        ? new Date(ph.tanggal).toLocaleDateString("id-ID", {
            day: "2-digit",
            month: "short",
            year: "numeric",
          })
...
async function approveKasbon(id, status) {
  ...
  toast(
    status === "approved" ? "✅ Kasbon disetujui" : "❌ Kasbon ditolak",
    "success",
  );
  renderKasbon();
}
async function bayarAngsuran(id) {
  ...
  const paymentHistory = [
    ...(p.paymentHistory || []),
    { jumlah: angsuran, tanggal: paymentAt, dicatatOleh: currentUser.nama },
  ];
  const update = {
    sudahBayar: newSudahBayar,
    lastPayment: paymentAt,
    paymentHistory,
  };
  if (sisa <= 0) update.status = "lunas";
  await db.collection("hrd_kasbon").doc(id).update(update);
  toast(
    `Angsuran ${formatCurrency(angsuran)} dibayar. Sisa: ${formatCurrency(sisa)}`,
    "success",
  );
  renderKasbon();
}
...
async function generateDefaultTunjangan() {
  ...
  for (const t of defaults) {
    await db
      .collection("hrd_tunjangan")
      .add({ ...t, createdAt: new Date().toISOString() });
  }
...
async function renderInsentif() {
  ...
  if (!items.length)
    h =
      '<tr><td colspan="7" class="text-center">Belum ada data insentif</td></tr>';
  else
...
async function generateInsentifFromKPI() {
  if (
    !confirm(
      "Generate insentif untuk semua karyawan berdasarkan data KPI terbaru?",
    )
  )
    return;
  const kSnap = await db
...
function calcTax() {
  ...
  const nettoTahunan = Math.max(
    0,
    (bruto - bpjsKesAuto - bpjsTKAuto) * 12 - ptkp,
  );
  let pphAuto = 0;
  if (nettoTahunan <= 60000000) pphAuto = nettoTahunan * 0.05;
  else if (nettoTahunan <= 250000000)
    pphAuto = 3000000 + (nettoTahunan - 60000000) * 0.15;
  else if (nettoTahunan <= 500000000)
    pphAuto = 3000000 + 28500000 + (nettoTahunan - 250000000) * 0.25;
  else
    pphAuto = 3000000 + 28500000 + 62500000 + (nettoTahunan - 500000000) * 0.3;
  const pphBulananAuto = Math.round(pphAuto / 12);
  // Use manual values if user edited them, otherwise auto
  const bpjsKes =
    Number(document.getElementById("tcBpjsKes")?.value) || bpjsKesAuto;
  const bpjsTK =
    Number(document.getElementById("tcBpjsTK")?.value) || bpjsTKAuto;
  const pphBulanan =
    Number(document.getElementById("tcPPH")?.value) || pphBulananAuto;
