...
async function doGenerateAllGaji() {
  const incTunjCuti =
    document.getElementById("genIncTunjCuti")?.checked || false;
  ...
  closeModalDirect();
  if (!confirm("Konfirmasi: Generate slip gaji untuk semua karyawan aktif?"))
    return;
  try {
    console.log(
    ...
    kSnapAll.forEach((d) => {
      ...
      if (data.status === "aktif")
        kDocs.push({ id: d.id, data: () => data, ref: d.ref });
    });
    const kSnap = {
      empty: kDocs.length === 0,
      docs: kDocs,
      size: kDocs.length,
    };
    if (kSnap.empty) {
    ...
    absenSnap.forEach((d) => {
      ...
      if (p.tipe === "pulang" && p.lembur && p.lemburJam)
        absenMap[uid].lembur += p.lemburJam;
    });
      ...
      if (incPPH) {
        ...
        if (penghasilanNetto <= 60000000) pphT = penghasilanNetto * 0.05;
          ...
          pphT =
            ...
            28500000 +
            62500000 +
            (penghasilanNetto - 500000000) * 0.3;
