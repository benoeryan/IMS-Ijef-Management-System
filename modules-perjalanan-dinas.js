...
function resolveGradeKey(grade) {
  ...
  if (g.includes("SENIOR") || g.includes("SUPERVISOR") || g.includes("LEADER"))
    return "SENIOR";
  return "STAFF";
}
...
async function loadBenefitConfig() {
  ...
  if (
    _benefitConfigCache &&
    now - _benefitConfigCacheTime < BENEFIT_CACHE_TTL
  ) {
...
function getGradeConfigSync(grade) {
  ...
  if (
    _benefitConfigCache &&
    _benefitConfigCache.benefit &&
    _benefitConfigCache.benefit[key]
  ) {
...
async function getUserGrade() {
  ...
  try {
    ...
    if (
      posisi.includes("SUPERVISOR") ||
      posisi.includes("SENIOR") ||
      posisi.includes("LEADER")
    ) {
...
function getGradePeraturanSync(grade) {
  ...
  if (
    _benefitConfigCache &&
    _benefitConfigCache.peraturan &&
    _benefitConfigCache.peraturan[key]
  ) {
...
async function showSPPDTab(tab) {
  ...
  document.querySelectorAll("#sppdTabs .tab").forEach((t) => {
    if (tab === "daftar" && t.textContent.includes("Daftar"))
      t.classList.add("active");
    else if (tab === "prosedur" && t.textContent.includes("Prosedur"))
      t.classList.add("active");
    else if (tab === "uang-muka" && t.textContent.includes("Uang Muka"))
      t.classList.add("active");
    else if (tab === "laporan" && t.textContent.includes("Laporan"))
      t.classList.add("active");
...
async function loadSPPDDaftar(el) {
  ...
  docs.forEach(function (p) {
    ...
    if (!isPortal && hasAccess(3) && !hasAccess(4)) {
      if (
        p.userId !== currentUser.id &&
        (p.departemen || "").toLowerCase() !==
          (currentUser.departemen || "").toLowerCase()
      )
        return;
    }
    hasData = true;
    var durasi =
      p.tanggalMulai && p.tanggalSelesai
        ? Math.ceil(
            (new Date(p.tanggalSelesai) - new Date(p.tanggalMulai)) /
              (1000 * 60 * 60 * 24) +
              1,
          ) + " hari"
        : "-";
    ...
    h += "<td>";
    h +=
      '<button class="btn btn-xs btn-info" onclick="viewSPPD(\'' +
      p.id +
      "')\">👁️</button> ";
    if (!isBOD && hasAccess(3) && p.status === "pending") {
      h +=
        ...
        "')\">✅</button> ";
      h +=
        '<button class="btn btn-xs btn-danger" onclick="rejectSPPD(\'' +
        p.id +
        "')\">❌</button> ";
    }
    if (p.status === "approved") {
      h +=
        '<button class="btn btn-xs btn-primary" onclick="cetakSPPD(\'' +
        p.id +
        "')\">🖨️</button> ";
    }
    if (!isBOD && hasAccess(6)) {
      h +=
        '<button class="btn btn-xs btn-warning" onclick="editSPPD(\'' +
        p.id +
        "')\">✏️</button> ";
      h +=
  ...
  if (!hasData)
    h +=
      '<tr><td colspan="8" class="text-center">Belum ada data SPPD</td></tr>';
...
function updateSPPDBulkUI() {
  ...
  if (masterCb)
    masterCb.checked = items.length > 0 && checked.length === items.length;
}
...
async function bulkApproveSPPD() {
  ...
  for (var i = 0; i < pendingIds.length; i++) {
    ...
    var linkSnap = await db
      .collection("hrd_dinas_luar")
      .where("noSPPD", "==", p.noSPPD)
      .get();
...
async function bulkDeleteSPPD() {
  ...
  if (
    !confirm(
      "Hapus " +
        ids.length +
        " SPPD yang dipilih? Tindakan ini tidak bisa dibatalkan.",
    )
  )
    return;
...
async function loadSPPDProsedur(el) {
  ...
  if (hasAccess(6)) {
    ...
    const getCmpBen = (g, field) => {
      if (fsConfig && fsConfig.benefit && fsConfig.benefit[g])
        return fsConfig.benefit[g][field];
...
async function saveBenefitConfig() {
  ...
  for (const g of allGrades) {
    const uangHarian =
      parseInt(document.getElementById("cfg_uangHarian_" + g)?.value) || 0;
    const maxTransport =
      parseInt(document.getElementById("cfg_maxTransport_" + g)?.value) || 0;
    const maxHotel =
      parseInt(document.getElementById("cfg_maxHotel_" + g)?.value) || 0;
    const kelasHotel =
      document.getElementById("cfg_kelasHotel_" + g)?.value || "";
    const maxMakan =
      parseInt(document.getElementById("cfg_maxMakan_" + g)?.value) || 0;
    const uangSaku =
      parseInt(document.getElementById("cfg_uangSaku_" + g)?.value) || 0;
    const persenUangMuka =
      parseInt(document.getElementById("cfg_persenUangMuka_" + g)?.value) || 0;
    const maxDurasi =
      parseInt(document.getElementById("cfg_maxDurasi_" + g)?.value) || 0;
    const batasLaporan =
      parseInt(document.getElementById("cfg_batasLaporan_" + g)?.value) || 0;
    const alurApproval =
      document.getElementById("cfg_alurApproval_" + g)?.value || "";
...
function cekBatasGrade() {
  ...
  const warnings = [];
  const transport =
    parseInt(document.getElementById("sppdBiayaTransport").value) || 0;
  const akomodasi =
    parseInt(document.getElementById("sppdBiayaAkomodasi").value) || 0;
  const makan = parseInt(document.getElementById("sppdBiayaMakan").value) || 0;
  if (transport > cfg.maxTransport)
    warnings.push(
      "Transport melebihi batas grade (max " +
...
async function simpanSPPD(noSPPD) {
  ...
  const _hari = Math.ceil(
    (new Date(data.tanggalSelesai) - new Date(data.tanggalMulai)) /
      (1000 * 60 * 60 * 24) +
      1,
  );
  ...
  if (_hari > 0) {
    ...
    if (_malam > 0 && data.biayaAkomodasi > gradeConfig.maxHotel * _malam)
      _exceeded = true;
    if (data.biayaMakan > (gradeConfig.maxMakan + gradeConfig.uangSaku) * _hari)
      _exceeded = true;
    if (
      _exceeded &&
      !confirm("Estimasi melebihi batas grade Anda, tetap ajukan?")
    )
      return;
  }
  ...
  try {
    ...
    console.warn(
      "Failed to update SPPD with dinasLuarId, cross-reference may be incomplete:",
      e,
    );
  }
  await sendNotification(
...
async function simpanEditSPPD(id) {
  const data = {
    ...
    biayaTransport:
      parseInt(document.getElementById("editSppdBiayaTransport").value) || 0,
    biayaAkomodasi:
      parseInt(document.getElementById("editSppdBiayaAkomodasi").value) || 0,
    biayaMakan:
      parseInt(document.getElementById("editSppdBiayaMakan").value) || 0,
    biayaLain:
      parseInt(document.getElementById("editSppdBiayaLain").value) || 0,
    ...
    const akomodasiStatus =
      malam > 0 && p.biayaAkomodasi <= maxAkomodasi
        ...
        : malam === 0
          ...
          : "color:var(--danger)";
    const makanStatus =
      p.biayaMakan <= maxMakan ? "color:var(--success)" : "color:var(--danger)";
...
async function loadSPPDUangMuka(el) {
  ...
  if (!hasData)
    h +=
      '<tr><td colspan="5" class="text-center">Belum ada pengajuan uang muka</td></tr>';
  h += "</tbody></table></div></div>";
  el.innerHTML = h;
}
...
async function simpanUangMukaDinasFromSelect() {
  ...
  const noSPPD =
    sel.options[sel.selectedIndex]?.getAttribute("data-nosppd") || "";
...
async function loadSPPDLaporan(el) {
  ...
  if (!hasData)
    h += '<tr><td colspan="6" class="text-center">Belum ada laporan</td></tr>';
  h += "</tbody></table></div></div>";
  el.innerHTML = h;
}
...
async function simpanLaporanDinasFromSelect() {
  ...
  const noSPPD =
    sel.options[sel.selectedIndex]?.getAttribute("data-nosppd") || "";
  const tujuan =
    sel.options[sel.selectedIndex]?.getAttribute("data-tujuan") || "";
  if (!sppdId) return toast("Pilih SPPD dulu", "warning");
...
async function loadSPPDReimbursement(el) {
  ...
  if (!hasData)
    h +=
      '<tr><td colspan="7" class="text-center">Belum ada reimbursement</td></tr>';
  h += "</tbody></table></div></div>";
  el.innerHTML = h;
}
